...
__device__ void calculate_laplacian(float *image, float *jacobian, int w, int h, int nc, float tau) {
    ...
    if (x < w && y < h && c < nc) {
        ...
        jacobian[idx] = ((x+1) < (w) ? 1.0f : 0.0f) * ( image[idx + 1] - temp_uxy) +
        		   ((x) > 0 ? 1.0f : 0.0f) * ( image[idx - 1] - temp_uxy) +
        		   ((y+1) < (h) ? 1.0f : 0.0f) *( image[idx + w] - temp_uxy) +
        		   ((y) > 0 ? 1.0f : 0.0f ) * ( image[idx - w] - temp_uxy);
    }
}
